...
training_cmd_PT_template = "python -m src.jobs.gen_train_cmds_OmniLearned -name FixBugSmallDataset_E_avail_Log1PLoss_PT_dscap{dscap}_Evts_seed_{dscapseed} --regress-E-available-no-muon -nw 10  --loss-type log1p --use-pretrained pretrain_m --run --dataset-cap {dscap} --dataset-cap-seed {dscapseed}"

for dscap in [10000, 50000, 100000, 500000]:
    for dscapseed in [42, 43, 44]:
