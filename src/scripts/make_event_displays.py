...
human_readable_current = {1: "CC", 2: "NC"}  # mc_current in the masterAnaDev
...
parser.add_argument(
    ...
    type=int,
    required=False,
    help="Max number of events to plot per interaction type",
    default=10,
)
            ...
            if len(available_idx) > 0:
                ...
                for event in events:
                    print(
                        ...
                        len(events),
                        " for ",
                        human_readable_current[key],
                        " ",
                        human_readable_intType[int_type_key],
                    )
                    mc_enu = enu_mc[event]
                    output_dir = Path(
                        os.path.join(
                    ...
                    fig = event_display(
                        ...
                        blobs=blob,
                        prongs=prong,
                        title=f"{human_readable_current[key]} {human_readable_intType[int_type_key]} event {event} Enu_MC={mc_enu}",
                    )
