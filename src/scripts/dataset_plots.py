...
for playlist in [
    ...
    DATASETS[playlist] = (
        f"/scratch/MINERvA/raw_data/MediumEnergy_FHC_StandardMC_Playlist/{playlist}"
    )
...
def print_statistics(result):
    ...
    print(
        f"{'Playlist':<12} {'Object':<15} {'Min':<6} {'Max':<6} {'Mean':<8} {'Median':<8} {'P95':<8}"
    )
    print("-" * 80)
    ...
    for playlist in sorted(result.keys()):
        for obj_name, key in object_types:
            if key in result[playlist]:
                data = result[playlist][key]
                print(
                    f"{playlist:<12} {obj_name:<15} {np.min(data):<6.0f} {np.max(data):<6.0f} "
        ...
        if root_file.endswith(".root"):
            ...
            with uproot.open(os.path.join(DATASETS[playlist], root_file)) as f:
                ...
                if not len(result[playlist]):
                    ...
                    result[playlist] = merge_histograms_dict(
                        result[playlist], get_histograms(master_ana_dev)
                    )
            root_file_count += 1
...
for playlist in result:
    ...
    mc_part_density = result[playlist]["N_mc_part"][0] / np.sum(
        result[playlist]["N_mc_part"][0]
    )
    prong_density = result[playlist]["N_prong"][0] / np.sum(
        result[playlist]["N_prong"][0]
    )
    blob_density = result[playlist]["N_blob"][0] / np.sum(result[playlist]["N_blob"][0])
    photons_density = result[playlist]["N_photons"][0] / np.sum(
        result[playlist]["N_photons"][0]
    )
    muons_density = result[playlist]["N_muons"][0] / np.sum(
        result[playlist]["N_muons"][0]
    )
    all_density = result[playlist]["N_all"][0] / np.sum(result[playlist]["N_all"][0])

    ax_n_objects[0, 0].stairs(
        mc_part_density, result[playlist]["N_mc_part"][1], label=playlist, alpha=0.5
    )
    ax_n_objects[0, 1].stairs(
        prong_density, result[playlist]["N_prong"][1], label=playlist, alpha=0.5
    )
    ax_n_objects[1, 0].stairs(
        blob_density, result[playlist]["N_blob"][1], label=playlist, alpha=0.5
    )
    ax_n_objects[1, 1].stairs(
        photons_density, result[playlist]["N_photons"][1], label=playlist, alpha=0.5
    )
    ax_n_objects[2, 0].stairs(
        muons_density, result[playlist]["N_muons"][1], label=playlist, alpha=0.5
    )
    ax_n_objects[2, 1].stairs(
        all_density, result[playlist]["N_all"][1], label=playlist, alpha=0.5
    )
