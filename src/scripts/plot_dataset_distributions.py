...
Reads preprocessed playlists (train/val/test ``*.pb`` files) and writes histograms for:
  ...
  - poster figures in ``<out-dir>/poster/`` (token multiplicity, energy comparison)
...
_POSTER_CACHE_OMIT_KEYS = frozenset({"jet_energy_gev", "incoming_e_gev"})
...
def _save_poster_fig(fig, out_path: Path):
...
def _write_poster_plots(cache: dict, poster_dir: Path):
    tag = cache["tag"]
...
def main():
    ...
    if args.poster_from_cache:
        ...
        _write_poster_plots(cache, poster_dir)
    ...
    _write_poster_plots(poster_cache, poster_dir)
    ...
    if not args.no_jetclass2 and jc_cache is not None:
        print(f"  joint plots in {poster_dir / 'joint'}")
