...
def print_statistics(baselines, playlist_name=""):
    ...
    for name in baseline_names:
        ...
        if n_valid > 0:
            ...
            print(
                f"  Mean: {valid_values.mean():.2f} MeV ({valid_values.mean()/1000:.2f} GeV)"
            )
            print(
                f"  Std:  {valid_values.std():.2f} MeV ({valid_values.std()/1000:.2f} GeV)"
            )
            print(
                f"  Min:  {valid_values.min():.2f} MeV ({valid_values.min()/1000:.2f} GeV)"
            )
            print(
                f"  Max:  {valid_values.max():.2f} MeV ({valid_values.max()/1000:.2f} GeV)"
            )
            print(
                f"  Median: {np.median(valid_values):.2f} MeV ({np.median(valid_values)/1000:.2f} GeV)"
            )
...
def plot_distributions(baselines, output_dir=None, playlist_name=""):
    ...
    title = (
        f"Neutrino Energy Baselines - {playlist_name}"
        if playlist_name
        else "Neutrino Energy Baselines"
    )
    ...
    if output_dir:
        output_path = (
            Path(output_dir) / f"{playlist_name}_distributions.png"
            if playlist_name
            else Path(output_dir) / "distributions.png"
        )
...
def plot_comparisons(baselines, output_dir=None, playlist_name=""):
    ...
    title = (
        f"Baseline Comparisons - {playlist_name}"
        if playlist_name
    ...
    if output_dir:
        output_path = (
            Path(output_dir) / f"{playlist_name}_comparisons.png"
            if playlist_name
            else Path(output_dir) / "comparisons.png"
        )
        plt.savefig(output_path, dpi=150, bbox_inches="tight")
        print(f"Saved plot to {output_path}")
    else:
...
def main():
    ...
    parser.add_argument(
        "--playlist", type=str, default="", help="Playlist name for labeling"
    )
    parser.add_argument(
        "--no-plots", action="store_true", help="Skip plotting, only print statistics"
    )
