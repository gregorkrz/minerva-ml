...
class FourierPositionalEncoding(nn.Module):
    def __init__(self, coord_dim: int, num_bands: int = 16, max_freq: float = 10.0):
        ...
        self.register_buffer(
            "freq_bands", torch.linspace(1.0, max_freq, num_bands), persistent=False
        )
...
class AbsolutePositionalMLP(nn.Module):
    def __init__(
        self, coord_dim: int, d_model: int, num_bands: int = 16, max_freq: float = 10.0
    ):
        super().__init__()
        self.fourier = FourierPositionalEncoding(
            coord_dim, num_bands=num_bands, max_freq=max_freq
        )
...
class MixedFeatureEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Continuous pathway
        self.cont_norm = (
            nn.LayerNorm(cont_dim) if (use_cont_layernorm and cont_dim > 0) else None
        )
        if cont_dim > 0:
        ...
        self.cat_embs = (
            nn.ModuleList([nn.Embedding(nc, cat_emb_dim) for nc in cat_num_classes])
            if len(cat_num_classes) > 0
            else nn.ModuleList()
        )
    ...
    def forward(
        ...
        if self.cat_proj is not None:
            if cats is None:
                raise ValueError(
                    "cats must be provided because categorical fields were configured."
                )
            if len(cats) != len(self.cat_embs):
                raise ValueError(
                    f"Expected {len(self.cat_embs)} categorical tensors, got {len(cats)}."
                )
